...
namespace ReportTools.Editor.Scripts.ApkParser
{
    public class ApkManifest
    {
        ...
        public string ReadManifestFileIntoXml(byte[] manifestFileData)
        {
            ...
            const int sitOff = 0x24; // Offset of start of StringIndexTable
            ...
            for (var i = xmlTagOff; i < manifestFileData.Length - 4; i += 4)
            {
                if (LEW(manifestFileData, i) == StartTag)
                {
                    xmlTagOff = i; 
                    break;
                }
            ...
            while (off < manifestFileData.Length)
            {
                ...
                {
                    Prt("  Unrecognized tag code '" + tag0.ToString("X") + "' at offset " + off);
                    break;
                }
        ...
        private string CompXmlStringAt(byte[] arr, int strOff)
        {
            ...
            return System.Text.Encoding.GetEncoding(_isUtf8 ? "UTF-8" : "UTF-16").GetString(chars);
        ...
        private int LEW(byte[] arr, int off)
        {
            ...
            return (int) (((uint) arr[off + 3]) << 24 & 0xff000000 | ((uint) arr[off + 2]) << 16 & 0xff0000 |
